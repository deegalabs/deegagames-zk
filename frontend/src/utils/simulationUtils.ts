...
export async function getSimulationSourceAddress(avoidAddresses: string[] = []): Promise<string> {
  ...
  throw new Error(
    'Nenhuma conta válida para simular a transação. Execute no frontend: cd frontend && bun run setup. Reinicie o dev server e funde as duas contas na testnet (Friendbot).'
  );
}
