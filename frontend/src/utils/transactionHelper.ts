...
export async function signAndSendViaLaunchtube(
  ...
  if (typeof tx !== 'string' && 'simulate' in tx) {
    ...
    try {
      ...
      console.log('[PokerZK] tx sent', sent?.getTransactionResponse ? 'has response' : 'simulation only');
      return sent;
    } catch (err: any) {
      ...
      // In those cases, the SDK requires `force: true` to sign and send anyway.
        ...
        try {
          ...
          console.log('[PokerZK] tx sent (force)', sent?.getTransactionResponse ? 'has response' : 'simulation only');
          return sent;
        } catch (forceErr: any) {
